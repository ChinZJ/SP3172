...
public class Board {
                ...
                for (int dy: shiftsSeeds) {
                    if (j + dy >= 0 && j + dy < this.boardLength && i + dx >= 0 && i + dx < this.boardLength) {
                        ...
                        if (targetTile.nSeeds.containsKey(dist)) {
                            ...
                            prob = targetTile.nSeedsProb.get(dist);
                        } else {
                        ...
                        if (dx < 0 ) { // x negative
                            if (dy < 0) { // both negative
                                prob.add(targetTile.distProbs.get(-dx) * targetTile.distProbs.get(-dy));
                            } else { //x negative, y positive
                                prob.add(targetTile.distProbs.get(-dx) * targetTile.distProbs.get(dy));
                            }
                        } else { // x positive
                            if (dy < 0) { // x positive, y negative
                                prob.add(targetTile.distProbs.get(dx) * targetTile.distProbs.get(-dy));
                        ...
                        targetTile.nSeedsProb.put(dist, prob);
                    }
                }
            }
        }
            ...
            for (int i = 0; i < numJuvenile; i++) { // Name not changed due to laziness
                while (true) {
                    ...
                    if (this.board.get(ind).oldAdult == null) { //check if empty
                        Adult adult = new Adult(species, 0); // juvenile takes 9
    ...
    public void update() {
        ...
        ExecutorService executorTwo = Executors.newFixedThreadPool(12);
