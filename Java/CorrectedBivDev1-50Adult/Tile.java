...
public class Tile {
    ...
    Map<Integer, List<Double>> nSeedsProb; // key: distance, value: list of probabilities for those tiles
    ...
    public HashMap<Species, Double> countNeighborsNDD() { 
        ...
        HashMap<Species, Double> neighborMap = new HashMap<>();
        double count = 0;
        ...
        for (int i = 0; i < this.nNDD.size(); i++) {
            ...
            for (Species key : neighb.oldNeighMap.keySet()) {
                double sum = neighb.oldNeighMap.get(key).first + (neighb.oldNeighMap.get(key).second / 50.0);
                count += sum;
        ...
        for (Map.Entry<Integer, List<Tile>> tiles : nSeeds.entrySet()) {
            ...
            for (Tile t : lst) {
                if (t.oldAdult != null) { // there exists an adult
                    ...
                    count += this.nSeedsProb.get(distance).get(index);
                    if (seedNeighbors.containsKey(neighborAdultSpecies)) {
                        seedNeighbors.put(neighborAdultSpecies, seedNeighbors.get(neighborAdultSpecies) + this.nSeedsProb.get(distance).get(index));
                    } else {
                        seedNeighbors.put(neighborAdultSpecies, this.nSeedsProb.get(distance).get(index));
                    }
                }
                index++;
            }
    ...
    public void updateExistingPlants() {
        ...
        HashMap<Species, Double> neighborMap = this.countNeighborsNDD();
        double totalNeighborAdults = neighborMap.get(Tile.DUMMY);
        ...
        for (int i = 0; i < oldPlantArray.size(); i++) {
			...
			double conNeighbors = 0.0;
			double hetNeighbors = 0.0;
            if (oldPlant instanceof Juvenile) {
                conNeighbors = neighborMap.get(oldPlant.species) - (1 / 50.0); // note that we have to SUBTRACT itself
                hetNeighbors = totalNeighborAdults - conNeighbors - (1 / 50.0); // again, subtract ITSELF
            } else {
                conNeighbors = neighborMap.get(oldPlant.species) - 1; // note that we have to SUBTRACT itself
                hetNeighbors = totalNeighborAdults - conNeighbors - 1; // again, subtract ITSELF
            }
